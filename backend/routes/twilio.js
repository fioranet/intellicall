    ...
    try {
        ...
        if (!currentAgentId || !currentLeadId) {
            ...
            if (!inboundLead) {
                ...
                EmailService.trigger(currentUserId, 'leadCreated', { lead: inboundLead });
            }
            currentLeadId = inboundLead._id;

            // Trigger Inbound Call Webhook + Email
            ...
            WebhookService.trigger(currentUserId, 'inboundCall', inboundPayload);
            EmailService.trigger(currentUserId, 'inboundCall', inboundPayload);
        }
...
router.post('/process', async (req, res) => {
    ...
    try {
        ...
        // free: <Say> then <Hangup/> means Twilio always finishes the closing line first.
        if (/\[\[END_CALL\]\]/.test(aiReply)) {
            const closing = aiReply.replace(/\[\[END_CALL\]\]/g, '').trim();
            console.log(`📴 [Twilio] [${callSid}] Agent ended the call`);
            if (closing) response.say({ voice: agent.voice || 'Polly.Amy', language: twilioLang }, closing);
            response.hangup();
            res.type('text/xml');
            return res.send(response.toString());
        }
...
router.post('/status', async (req, res) => {
    ...
    try {
        ...
        // Twilio's CallDuration is carrier-observed and billed, so it is the authority and
        // this write is deliberately unconditional. Every other duration writer goes through
        // persistDurationIfUnset() so it can only fill a gap, never overwrite this value.
        ...
        let updatedLog = await CallLog.findOneAndUpdate(
            { callSid: CallSid },
            updateData,
            { returnDocument: 'after' }
        );

        // Terminal status but Twilio sent no CallDuration — derive one from startTime rather
        // than leaving the row reading "0s".
        if (updatedLog && !updatedLog.duration && normalizedStatus && TERMINAL_CALL_STATUSES.includes(normalizedStatus)) {
            const startedMs = updatedLog.startTime ? new Date(updatedLog.startTime).getTime() : 0;
            const derived = startedMs ? Math.max(0, Math.round((Date.now() - startedMs) / 1000)) : 0;
            if (derived > 0) {
                await CallLog.updateOne({ _id: updatedLog._id }, { $set: { duration: derived } });
                updatedLog.duration = derived;
            }
        }
        ...
        if (normalizedStatus === 'completed' && updatedLog) {
            ...
            WebhookService.trigger(updatedLog.userId, 'callCompleted', completedPayload);
            EmailService.trigger(updatedLog.userId, 'callCompleted', completedPayload);
        }
