...
// GET /api/admin/integrations - OAuth credentials for connector integrations
        ...
        res.status(200).json({
            ...
            data: {
                integrations: {
                    ...
                    hubspot: {
                        clientId: integrations.hubspot?.clientId || '',
                        clientSecret: integrations.hubspot?.clientSecret || '',
                        callbackUrl: integrations.hubspot?.callbackUrl || ''
                    },
                    slack: {
                        clientId: integrations.slack?.clientId || '',
                        clientSecret: integrations.slack?.clientSecret || '',
                        callbackUrl: integrations.slack?.callbackUrl || ''
                    }
                }
            }
        });
...
// PATCH /api/admin/integrations - Update OAuth credentials
router.patch('/integrations', auth, isAdmin, async (req, res) => {
    try {
        const update = {};
        const allowed = ['clientId', 'clientSecret', 'callbackUrl'];
        for (const provider of ['google', 'hubspot', 'slack']) {
            const incoming = req.body?.[provider];
            if (!incoming || typeof incoming !== 'object') continue;
            for (const field of allowed) {
                if (typeof incoming[field] === 'string') {
                    update[`integrations.${provider}.${field}`] = incoming[field].trim();
                }
            }
        }
...
// Blank (transparent) defaults — what shows when no custom branding is set.
// The original logo files are intentionally blank so the stock logo never
// flashes during the load delay; deleting a custom asset reverts to blank.
...
// The stock (original) logos, kept aside under /images/originals/. Used only by
// the explicit "Reset branding to defaults" action to restore the stock look.
const BRANDING_ORIGINALS = {
    logoLight: '/images/originals/logo_black.png',
    logoDark: '/images/originals/logo_white.png',
    favicon: '/images/originals/favicon.ico'
};
...
router.post('/branding/reset', auth, isAdmin, async (req, res) => {
    try {
        ...
        // Reset all branding fields — restore the stock (original) logos.
...
// GET /api/admin/testimonials - Public: fetch active testimonials for landing page
...
// GET /api/admin/testimonials/all - Admin: fetch all testimonials including inactive
router.get('/testimonials/all', auth, isAdmin, async (req, res) => {
    try {
...
router.post('/testimonials', auth, isAdmin, async (req, res) => {
    try {
...
// PATCH /api/admin/testimonials/:id - Admin: update testimonial
router.patch('/testimonials/:id', auth, isAdmin, async (req, res) => {
    try {
