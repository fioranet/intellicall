...
router.post('/', auth, requireActivePlan, checkLimit('leads'), async (req, res) => {
    ...
    try {
        ...
        // Trigger Email
        EmailService.trigger(req.user._id, 'leadCreated', { lead });
        // Sync to HubSpot (fire-and-forget; never blocks lead creation)
        HubSpotService.upsertContactFromLead(req.user._id, lead).catch(() => { });
...
router.post('/bulk', auth, requireActivePlan, checkLimit('leads'), async (req, res) => {
    ...
    try {
        ...
        // Sync each new lead to HubSpot (fire-and-forget; never blocks the response)
...
router.get('/google-sheets/list', auth, async (req, res) => {
    try {
        ...
        const drive = await getDriveClient(settings.googleSheetsAccessToken, settings.googleSheetsRefreshToken);
...
router.get('/google-sheets/sheets/:spreadsheetId', auth, async (req, res) => {
    try {
        ...
        const sheets = await getSheetsClient(settings.googleSheetsAccessToken, settings.googleSheetsRefreshToken);
