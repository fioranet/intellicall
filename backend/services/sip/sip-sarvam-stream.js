...
const { SarvamBridge } = require('../sarvam/sarvam-bridge');
...
/**
 * Outbound playout buffer bound, in packets. Bulbul synthesises far faster than realtime, so a
 * long opening message legitimately sits here for tens of seconds — this is a playout buffer,
 * not a jitter buffer, and the old 500-packet (10s) cap was routinely exceeded by normal
 * replies. 3000 packets is 60 seconds.
 */
...
/**
 * SIP/Asterisk ↔ Sarvam AI adapter.
 *
 * Same public contract as SipVoiceStream / SipDeepgramAgentStream (start, setBridgeReady,
 * setOnTestComplete, injectAudioSpeech, cleanup, campaignId) so sip-manager only branches at
 * construction. Caller RTP (µ-law 8k) → Sarvam STT; agent audio (µ-law 8k) → paced 20ms RTP.
 *
 * Unlike the Deepgram Voice Agent adapter, this one DOES handle test calls: a Sarvam-only
 * account has no ElevenLabs key, so the test phrase is spoken through Sarvam's Bulbul TTS.
 */
class SipSarvamStream {
    constructor({ userId, agentId, leadId, campaignId, direction, callId, rtpPort, testCall = false, testPhrase = '' }) {
        ...
        this._testCompleteScheduled = false;

        // RTP state (mirrors SipDeepgramAgentStream)
        this.udpSocket = null;
        ...
        this._rtpQueue = [];
        this._rtpTimer = null;
        this._rtpNextSendAt = 0;   // wall-clock deadline for the next packet
        this._rtpOverflowLogged = false;
    ...
    async start() {
        try {
            ...
            if (!this.settings || !this.agent) {
                ...
                console.error(`❌ [SIP Sarvam] ${msg}`);
                this._pushError('system', 'missing_config', msg);
                return false;
            }
            // Sarvam is fully self-contained — only a Sarvam key is required.
            if (!this.settings.sarvamKey) {
                const msg = 'Missing Sarvam API key. Please configure it in Settings.';
                console.error(`❌ [SIP Sarvam] ${msg}`);
                this._pushError('system', 'missing_keys', msg);
                return false;
            }

            this.bridge = new SarvamBridge({
                callId: this.callId, settings: this.settings, agent: this.agent, lead: this.lead,
                testCall: this.testCall, testPhrase: this.testPhrase,
            });
            ...
            // Agent audio is paced out at 20ms/packet, so the queue can hold seconds of speech
            // after Sarvam has finished generating it. The bridge needs that to know the caller
            // is still hearing the agent when it decides barge-in vs. backchannel.
            this.bridge.isPlaybackActive = () => this._rtpQueue.length > 0;
            this.bridge.onEndCall = () => this._scheduleEndCall();
            this.bridge.onError = (service, code, message) => this._pushError(service, code, message);

            // Cache KB content once per call (skip for test calls)
            if (!this.testCall && this.agent.knowledgeBaseId) {
                try {
                    ...
                    console.error(`[SIP Sarvam] [${this.callId}] KB preload failed:`, kbErr.message);
                }
            }
            ...
            await CallLog.findOneAndUpdate(
                { callSid: this.callId },
                {
                    ...
                    direction: this.direction, provider: 'sip', voiceEngine: 'sarvam',
            ...
            console.log(`✅ [SIP Sarvam] Active — call ${this.callId} on UDP :${this.rtpPort}`);

            if (!this.testCall && this.direction === 'inbound') {
            ...
            console.error('❌ [SIP Sarvam] Start failed:', err);
            this.cleanup();
            return false;
        }
    }
    ...
    /**
     * Called by sip-manager once the Asterisk bridge is established. Connecting here (not in
     * start()) avoids the Sarvam sockets idling while the call is still ringing.
     */
    setBridgeReady() {
        this.bridgeReady = true;
        this.answeredAt = Date.now();
        console.log(`🌉 [SIP Sarvam] [${this.callId}] Bridge ready — connecting Sarvam`);
    ...
    detachForTransfer() {
        ...
        this.onMediaTimeout = null;
        this.onEndCall = null;
        console.log(`🔇 [SIP Sarvam] [${this.callId}] Detached from the bridge — caller is with a human`);
    ...
    /**
     * The agent asked to end the call. Its closing line is still queued for playout, so
     * hang up only once the RTP queue drains — with a safety timer in case it never does.
     */
    _scheduleEndCall() {
        if (this._endCallPending || this._endCallFired) return;
        this._endCallPending = true;
        console.log(`📴 [SIP Sarvam] [${this.callId}] Agent requested end of call — waiting for playback to finish`);
    ...
    _cancelEndCall() {
        ...
        this._endCallPending = false;
        if (this._endCallSafetyTimer) { clearTimeout(this._endCallSafetyTimer); this._endCallSafetyTimer = null; }
        console.log(`🙅 [SIP Sarvam] [${this.callId}] End of call cancelled — caller spoke again`);
    }
    ...
    _fireEndCall(why) {
        ...
        if (this._endCallSafetyTimer) { clearTimeout(this._endCallSafetyTimer); this._endCallSafetyTimer = null; }
        console.log(`📴 [SIP Sarvam] [${this.callId}] Ending call (${why})`);
    ...
    _startRtpWatchdog() {
        ...
        this._rtpWatchdog = setInterval(() => {
            ...
            if (gap > RTP_SILENCE_TIMEOUT_MS) {
                console.warn(`📵 [SIP Sarvam] [${this.callId}] No caller RTP for ${gap}ms — treating as hangup`);
    ...
    async injectAudioSpeech(text) {
        ...
        console.log(`🔊 [SIP Sarvam] Injecting speech: ${text}`);
    ...
    async cleanup() {
        ...
        console.log(`🧹 [SIP Sarvam] [${this.callId}] cleanup | transcriptLen=${transcript.length}`);
        ...
        if (this.callId) {
            try {
                ...
                const updateData = {
                    ...
                    endTime, duration, provider: 'sip', voiceEngine: 'sarvam'
                };
                ...
                if (log && hadConversation && this.settings?.autoAnalysisEnabled) {
                    analyzeCallLog(log._id).catch(e => console.error('[SIP Sarvam] Analysis err:', e));
                }
                    ...
                    try {
                        ...
                        if (campaign && campaign.status === 'running') {
                            ...
                            if (finishedCalls >= totalLeads) {
                                ...
                                console.log(`[SIP Sarvam] Campaign ${log.campaignId} marked as completed (${finishedCalls}/${totalLeads} calls done)`);
                            }
                        }
                    } catch (campErr) {
                        console.error('[SIP Sarvam] Campaign completion check err:', campErr.message);
                    }
                }
            } catch (e) { console.error('[SIP Sarvam] Log save err:', e); }
        }
    }

    // ─── RTP transport (mirrors SipDeepgramAgentStream) ──────
        ...
        return new Promise((resolve, reject) => {
            ...
            this.udpSocket.on('message', (msg, rinfo) => {
                ...
                if (!this.remoteAddress) {
                    ...
                    console.log(`📡 [SIP Sarvam] [${this.callId}] RTP Source Identified: ${rinfo.address}:${rinfo.port}`);
                    this._startRtpWatchdog();
                }
                // Test calls only play a phrase — don't feed caller audio into STT.
                if (!this.testCall && msg.length > RTP_HEADER_SIZE && this.bridge) {
                    this.bridge.sendAudio(msg.slice(RTP_HEADER_SIZE));
                }
            });

            this.udpSocket.on('error', (err) => {
                console.error(`❌ [SIP Sarvam] UDP err port ${this.rtpPort}:`, err);
                if (!this.active) { reject(err); return; }
                console.error(`❌ [SIP Sarvam] [${this.callId}] UDP socket error during active call — cleaning up`);
                this.cleanup();
            });

            this.udpSocket.bind(this.rtpPort, '0.0.0.0', () => {
                console.log(`🎧 [SIP Sarvam] RTP listening on :${this.rtpPort}`);
                resolve();
            });
        });
    }
    ...
    _clearPlayback() {
        ...
        console.log(`🛑 [SIP Sarvam] [${this.callId}] Playback cleared (barge-in)`);
    }
    ...
    _sendRtp(mulawBuf) {
        ...
        if (this._rtpQueue.length > MAX_RTP_QUEUE) {
            ...
            if (!this._rtpOverflowLogged) {
                ...
                console.warn(`⚠️ [SIP Sarvam] [${this.callId}] RTP playout buffer at its ${MAX_RTP_QUEUE}-packet (${Math.round(MAX_RTP_QUEUE * RTP_PACKET_MS / 1000)}s) bound — dropping ${excess} oldest packet(s). The agent's reply is longer than the buffer.`);
            }
        }
    ...
    _drainRtpQueue() {
        if (this._rtpQueue.length === 0 || !this.udpSocket || !this.remoteAddress) {
            ...
            this._rtpNextSendAt = 0;
            this.isAISpeaking = false;
            // Test call: once the phrase has fully played out, hang up. Guard against
            // scheduling more than once, and re-check the queue is still empty when the
            // timer fires so an inter-chunk gap doesn't hang up mid-phrase.
            if (this.testCall && !this._testCompleteScheduled) {
                this._testCompleteScheduled = true;
                setTimeout(() => {
                    if (this._rtpQueue.length > 0) { this._testCompleteScheduled = false; return; }
            ...
            // Same shape for an agent-requested hangup: the closing line has now been sent.
        ...
        // whole call — which both delays playout and makes isPlaybackActive() report the agent
        // as still speaking long after it has finished.
        const now = Date.now();
...
module.exports = SipSarvamStream;
