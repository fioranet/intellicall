...
const { DeepgramAgentBridge } = require('../deepgram-agent/agent-bridge');
...
/**
 * Outbound playout buffer bound, in packets. The Voice Agent streams audio far faster than
 * realtime, so a long reply legitimately sits here for tens of seconds — this is a playout
 * buffer, not a jitter buffer, and the old 500-packet (10s) cap was routinely exceeded by
 * normal replies. 3000 packets is 60 seconds.
 */
...
/**
 * SIP/Asterisk ↔ Deepgram Voice Agent adapter.
 *
 ...
 * Caller RTP (mulaw 8k) → Deepgram Voice Agent; agent audio → paced 20ms RTP packets.
 * Test calls never use this class — sip-manager keeps them on classic SipVoiceStream.
 */
class SipDeepgramAgentStream {
    ...
    async start() {
        try {
            ...
            if (!this.settings || !this.agent) {
                ...
                console.error(`❌ [SIP DG Agent] ${msg}`);
                this._pushError('system', 'missing_config', msg);
                return false;
            }
            // Deepgram-managed LLM — OpenRouter key not required for this engine
            if (!this.settings.deepgramKey || !this.settings.elevenLabsKey) {
                const missing = [];
                if (!this.settings.deepgramKey) missing.push('Deepgram');
                if (!this.settings.elevenLabsKey) missing.push('ElevenLabs');
            ...
            if (this.agent.knowledgeBaseId) {
                try {
                    ...
                    console.error(`[SIP DG Agent] [${this.callId}] KB preload failed:`, kbErr.message);
                }
            }
            ...
            await CallLog.findOneAndUpdate(
                { callSid: this.callId },
                {
                    ...
                    direction: this.direction, provider: 'sip', voiceEngine: 'deepgram_agent',
            ...
            console.log(`✅ [SIP DG Agent] Active — call ${this.callId} on UDP :${this.rtpPort}`);
            ...
            console.error('❌ [SIP DG Agent] Start failed:', err);
            this.cleanup();
            return false;
        }
    }
     ...
     * The greeting is configured in Settings, so Deepgram speaks it as soon as it's live.
     */
    setBridgeReady() {
        this.bridgeReady = true;
        this.answeredAt = Date.now();
        console.log(`🌉 [SIP DG Agent] [${this.callId}] Bridge ready — connecting Voice Agent`);
    ...
    detachForTransfer() {
        ...
        this.onMediaTimeout = null;
        this.onEndCall = null;
        console.log(`🔇 [SIP DG Agent] [${this.callId}] Detached from the bridge — caller is with a human`);
    ...
    _scheduleEndCall(reason) {
        ...
        console.log(`📴 [SIP DG Agent] [${this.callId}] end_call requested (${reason || 'no reason'}) — waiting for playback to finish`);
    ...
    _cancelEndCall() {
        ...
        this._endCallPending = false;
        if (this._endCallSafetyTimer) { clearTimeout(this._endCallSafetyTimer); this._endCallSafetyTimer = null; }
        console.log(`🙅 [SIP DG Agent] [${this.callId}] End of call cancelled — caller spoke again`);
    }
    ...
    _fireEndCall(why) {
        ...
        if (this._endCallSafetyTimer) { clearTimeout(this._endCallSafetyTimer); this._endCallSafetyTimer = null; }
        console.log(`📴 [SIP DG Agent] [${this.callId}] Ending call (${why})`);
        if (this.onEndCall) this.onEndCall();
    }

    async injectAudioSpeech(text) {
        if (!this.active || !this.bridge) return;
        console.log(`🔊 [SIP DG Agent] Injecting speech: ${text}`);
    ...
    async cleanup() {
        ...
        console.log(`🧹 [SIP DG Agent] [${this.callId}] cleanup | transcriptLen=${transcript.length}`);
        ...
        if (this.callId) {
            try {
                ...
                const updateData = {
                    ...
                    endTime, duration, provider: 'sip', voiceEngine: 'deepgram_agent'
                };
                ...
                if (log && hadConversation && this.settings?.autoAnalysisEnabled) {
                    analyzeCallLog(log._id).catch(e => console.error('[SIP DG Agent] Analysis err:', e));
                }
                    ...
                    try {
                        ...
                        if (campaign && campaign.status === 'running') {
                            ...
                            if (finishedCalls >= totalLeads) {
                                ...
                                console.log(`[SIP DG Agent] Campaign ${log.campaignId} marked as completed (${finishedCalls}/${totalLeads} calls done)`);
                            }
                        }
                    } catch (campErr) {
                        console.error('[SIP DG Agent] Campaign completion check err:', campErr.message);
                    }
                }
            } catch (e) { console.error('[SIP DG Agent] Log save err:', e); }
        }
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.udpSocket.on('message', (msg, rinfo) => {
                ...
                if (!this.remoteAddress) {
                    ...
                    console.log(`📡 [SIP DG Agent] [${this.callId}] RTP Source Identified: ${rinfo.address}:${rinfo.port}`);
            ...
            this.udpSocket.on('error', (err) => {
                console.error(`❌ [SIP DG Agent] UDP err port ${this.rtpPort}:`, err);
                if (!this.active) { reject(err); return; }
                console.error(`❌ [SIP DG Agent] [${this.callId}] UDP socket error during active call — cleaning up`);
                this.cleanup();
            });

            this.udpSocket.bind(this.rtpPort, '0.0.0.0', () => {
                console.log(`🎧 [SIP DG Agent] RTP listening on :${this.rtpPort}`);
                resolve();
            });
        });
    }
    ...
    _startRtpWatchdog() {
        ...
        this._rtpWatchdog = setInterval(() => {
            ...
            if (gap > RTP_SILENCE_TIMEOUT_MS) {
                console.warn(`📵 [SIP DG Agent] [${this.callId}] No caller RTP for ${gap}ms — treating as hangup`);
    ...
    _clearPlayback() {
        ...
        console.log(`🛑 [SIP DG Agent] [${this.callId}] Playback cleared (barge-in)`);
    }
    ...
    _sendRtp(mulawBuf) {
        ...
        if (this._rtpQueue.length > MAX_RTP_QUEUE) {
            ...
            if (!this._rtpOverflowLogged) {
                ...
                console.warn(`⚠️ [SIP DG Agent] [${this.callId}] RTP playout buffer at its ${MAX_RTP_QUEUE}-packet (${Math.round(MAX_RTP_QUEUE * RTP_PACKET_MS / 1000)}s) bound — dropping ${excess} oldest packet(s). The agent's reply is longer than the buffer.`);
            }
        }
...
module.exports = SipDeepgramAgentStream;
