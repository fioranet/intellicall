...
const { DeepgramAgentBridge } = require('./agent-bridge');
...
/**
 * Media Streams ↔ Deepgram Voice Agent adapter.
 *
 * voice-stream.js delegates here when agent.voiceEngine === 'deepgram_agent':
 * inbound media frames → bridge; agent audio → media frames; barge-in → 'clear';
 * stop/close → CallLog + webhooks (mirrors classic cleanup).
 *
 * `transport` is 'twilio' for a real call and 'browser' for the in-browser agent
 * test, which speaks the same frame protocol. A browser session has no phone leg
 * to drop and is not a real call, so it closes the socket instead of calling the
 * Twilio REST API and writes nothing to CallLog.
 */
class TwilioDeepgramAgentSession {
    constructor({ ws, streamSid, callSid, userId, agentId, leadId, campaignId, direction, settings, agent, lead, transport = 'twilio' }) {
        ...
        // Twilio buffers and paces audio on its side, so there is no local queue to measure.
        // Estimate instead: µ-law 8k is 8 bytes per millisecond of speech.
        this._playoutUntil = 0;

        this.bridge = new DeepgramAgentBridge({ callId: callSid, settings, agent, lead });

        this.bridge.onAudio = (buf) => {
            if (this.ws.readyState !== WebSocket.OPEN || !this.streamSid) return;
            if (this.ws.bufferedAmount > 1024 * 1024) {
                console.warn(`⚠️ [DG Agent] [${this.callSid}] Dropping audio chunk — ws backpressure (buffered=${this.ws.bufferedAmount})`);
                return;
            }
        ...
        this.bridge.onBargeIn = () => {
            ...
            if (this._endCallTimer) {
                ...
                this._endCallTimer = null;
                console.log(`🙅 [DG Agent] [${this.callSid}] Scheduled hangup cancelled — caller spoke again`);
            }
            if (this.ws.readyState === WebSocket.OPEN) {
                this.ws.send(JSON.stringify({ event: 'clear', streamSid: this.streamSid }));
                console.log(`🛑 [DG Agent] [${this.callSid}] Twilio clear sent (barge-in)`);
            }
        };
    ...
    /**
     * The agent decided the conversation is over. AgentAudioDone only means Deepgram
     * finished SENDING the closing line — Twilio still has it buffered, so wait out the
     * estimated playout or the goodbye is cut off mid-word.
     */
    _hangupAfterPlayout() {
        if (this._endCallTimer || !this.callSid) return;
        const waitMs = Math.max(0, this._playoutUntil - Date.now()) + END_CALL_DRAIN_GRACE_MS;
        console.log(`📴 [DG Agent] [${this.callSid}] Agent ended the call — hanging up in ${waitMs}ms`);
        this._endCallTimer = setTimeout(async () => {
            ...
            try {
                ...
                console.error(`❌ [DG Agent] [${this.callSid}] Hangup failed:`, err.message);
            }
        }, waitMs);
    }
    ...
    async start() {
        ...
        if (this.agent.knowledgeBaseId) {
            try {
                ...
                console.error(`[DG Agent] [${this.callSid}] KB preload failed:`, kbErr.message);
            }
        }
        this.bridge.connect();
    }

    /** Twilio inbound media payload (base64 mulaw 8k) → Deepgram. */
    ...
    cleanup() {
        ...
        console.log(`🧹 [DG Agent] [${this.callSid}] cleanup | transcriptLen=${transcript.length}`);
        this.bridge.close();
        ...
        if (this.callSid) {
            ...
            const updateData = {
                ...
                direction: this.direction, voiceEngine: 'deepgram_agent',
            ...
            CallLog.findOneAndUpdate({ callSid: this.callSid }, updateData, { upsert: true, returnDocument: 'after' }).then(async (updatedLog) => {
                ...
                if (hadConversation && this.settings?.autoAnalysisEnabled) {
                    console.log(`[DG Agent] Auto-analysis triggered for call ${this.callSid}`);
                    analyzeCallLog(updatedLog._id).catch(e => console.error('[DG Agent] Analysis Error:', e));
                }
            ...
            }).catch(e => console.error('[DG Agent] Log Error:', e));
        }
    }
}

module.exports = { TwilioDeepgramAgentSession };
