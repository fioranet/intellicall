    ...
    static generateContent(event, data, appName = 'IntelliCallAI') {
        ...
        switch (event) {
            ...
            case 'leadCreated': {
                ...
                subject = `👤 New Lead: ${lead.name || lead.phone || 'Unknown'}`;
                body += `<p>A new lead has been added.</p>`;
                body += processData({
            ...
            case 'leadQualified': {
                ...
                subject = `✅ Lead Qualified${data.name ? `: ${data.name}` : ''}`;
                body += `<p>The AI has qualified a lead.</p>`;
                body += processData({
                    Score: analysis.qualificationScore ?? data.score ?? '—',
                    Reason: analysis.reason || '—',
                    Budget: analysis.budget || '—',
                    Timeline: analysis.timeline || '—',
                    'Next Steps': analysis.nextSteps || '—',
                    'AI Opinion': analysis.aiOpinion || '—',
                    Status: data.status || 'qualified'
                });
                break;
            }
            case 'inboundCall':
                subject = `📞 Inbound Call${data.phoneNumber ? ` from ${data.phoneNumber}` : ''}`;
                body += `<p>An inbound call has started.</p>`;
                body += processData({
                    'Phone Number': data.phoneNumber || data.from || 'Unknown',
                    Direction: data.direction || 'inbound',
                    Provider: data.provider || '—'
                });
                break;
            case 'callCompleted':
                subject = `✔️ Call Completed`;
                body += `<p>A call has finished.</p>`;
                body += processData({
                    Status: data.status || 'completed',
                    Duration: (data.duration !== undefined && data.duration !== null && data.duration !== '')
                        ? `${data.duration} sec` : '—',
