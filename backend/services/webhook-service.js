...
class WebhookService {
    ...
    static async trigger(userId, event, data) {
        // Fan out to Slack alongside the user's webhook. Slack is its own
        // service with independent enable/per-event config; failures are
        // logged inside SlackService and never propagate here.
        try {
            require('./slack-service').trigger(userId, event, data);
        } catch (_) { /* never let Slack break webhook delivery */ }

        // Same fan-out for WhatsApp notifications (Meta Cloud API).
        try {
            require('./whatsapp-service').trigger(userId, event, data);
        } catch (_) { /* never let WhatsApp break webhook delivery */ }

        // Same fan-out for the user's n8n workflow webhook.
        try {
            require('./n8n-service').trigger(userId, event, data);
        } catch (_) { /* never let n8n break webhook delivery */ }
